// ═══════════════════════════════════════════════════════════════════
//  BLNDR. — Cloudflare Pages _worker.js
//  Ubicación correcta: public/_worker.js
//  Cloudflare Pages lo detecta automáticamente en esta carpeta.
// ═══════════════════════════════════════════════════════════════════
...
async function makeToken(userId, secret) {
  ...
  return btoa(raw + ':' + (secret || 'blndr2025').slice(0, 8));
}
  ...
  async fetch(request, env, ctx) {
    ...
    if (path.startsWith('/api/')) {
      try {
        ...
        if (method === 'GET'    && path === '/api/me')                 return await handleGetMe(request, env);
        if (method === 'PUT'    && path === '/api/me')                 return await handleUpdateMe(request, env);
        if (method === 'GET'    && path === '/api/users')              return await handleGetUsers(request, env);
        if (method === 'GET'    && path === '/api/events')             return await handleGetEvents(request, env);
        if (method === 'POST'   && path === '/api/events')             return await handleCreateEvent(request, env);
        if (method === 'DELETE' && path.startsWith('/api/events/')) {
          const id = parseInt(path.split('/')[3]);
          return await handleDeleteEvent(request, env, id);
        }
...
async function handleRegister(request, env) {
  ...
  if (clean.length < 3) return err('Usuario inválido (solo letras, números y _)');

  const existing = await env.DB.prepare('SELECT id FROM users WHERE username = ?').bind(clean).first();
  ...
  ).bind(name.trim(), clean, hashed, color || '#c8ff00', avatar || '🦋').first();

  const token = await makeToken(result.id, env.JWT_SECRET);
  return json({
    token,
    user: { id: result.id, name: name.trim(), username: clean, color: color || '#c8ff00', avatar: avatar || '🦋' }
  }, 201);
}
...
async function handleLogin(request, env) {
  ...
  const token = await makeToken(row.id, env.JWT_SECRET);
  return json({
    token,
...
async function handleUpdateMe(request, env) {
  ...
  if (!updates.length) return err('Nada para actualizar');
...
async function handleCreateEvent(request, env) {
  ...
  return json({
    event: {
      id: result.id, user_id: userId, type, description: description.trim(),
